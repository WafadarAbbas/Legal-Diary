...
const EditBench = (props) => {
      ...
      if (id) {
        try {
          const response = await ApiCall({
            url: `https://localhost:44311/api/services/app/Bench/GetBenchMasterDetailForEdit?Id=${id}`,
            method: "GET",
          });
          if (response?.data?.result) {
            const benchDetail = response.data.result.benchMainDetailEdit;
            const officers = response.data.result.listOfBenchDetailsDto;

            console.log("Fetched Bench Data:", benchDetail);
            console.log("Fetched Officer List:", officers);

            formik.setValues({
              branchId: benchDetail.branchId || "",
              benchCode: benchDetail.benchCode || "",
              courtId: benchDetail.courtId || "",
              benchOfficerNo: benchDetail.benchOfficerNo || 0,
              benchStartDate: benchDetail.benchStartDate?.split("T")[0] || "",
              benchEndDate: benchDetail.benchEndDate?.split("T")[0] || "",
              benchStatus: benchDetail.benchStatus || false,
              presidingOfficerId: benchDetail.presidingOfficerId || 0,
              courtCourtCode: benchDetail.courtCourtCode || " ", // New field
              branchBranchName: benchDetail.branchBranchName || " ",
            });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await ApiCall({
          url: `https://localhost:44311/api/services/app/Bench/Update`, 
          method: "PUT", 
          data: finalValues, 
        });
